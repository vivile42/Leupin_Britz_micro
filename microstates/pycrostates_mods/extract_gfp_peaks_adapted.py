...
def extract_gfp_peaks(
    ...
    if isinstance(inst, BaseRaw):
        ...
        ind_peaks,gfp = _extract_gfp_peaks(data, min_peak_distance)
        ...
        for k in range(len(inst)):  # pylint: disable=consider-using-enumerate
            ...
            if last_gfp_peak:
                try:
                    ...
                    raise ValueError('at least one missing gfp')

                    #find maxima after minima
...
def _extract_gfp_peaks(
    ...
    if len(peaks)==0:
        #finds gfp peak max after minima, usually the last frame in epoch in which it failed
        min_idx=np.argmin(gfp)
        if min_idx>=(len(gfp)-1):
            peaks=[np.argmax(gfp)]
            plot_gfp_peaks(gfp,peaks)
        else:
            peaks=[np.argmax(gfp[min_idx:])+min_idx]


    return peaks,gfp

def plot_gfp_peaks(gfp,peak):
    x_cord=np.arange(len(gfp))

    plt.plot(x_cord,gfp)
    for pk in peak:
        plt.vlines(pk,min(gfp),max(gfp),'r','--')